				...
				.ajax({
					...
					success : function(result) {
						...
						for (i = 0; i < json['chuong'].length; i++) {
							...
							$("#cau_truc_cau_de2")
									.append(
											"<div class='col-md-6'><label>Chương&emsp; </label><input class='form-control' type='text' readonly='readonly' value='"
...
function thay_doi_trang_thai(e) {
	var s = $(e).children("option").filter(":selected").val();
	var array = s.split("|");
	var trang_thai = array[0];
	var maCV = array[1];
...
function thay_doi_tien_do_cv(e){
	var s = $(e).children("option").filter(":selected").val();
	var array = s.split("|");
	var trang_thai = array[1];
	var maCV = array[0];
	var form_data ={maCV:maCV,trang_thai:trang_thai}
	$.ajax({
		url : '/congviec/thaydoitiendocongviec',
		type : 'POST',
		dataType : 'text',
		data : form_data,
		success : function(result) {
			if(result=='Cập nhât thành công'||result=='Cập nhật thất bại'){
				
				swal(result);
			}else{
				window.location='/duyet-cau-hoi/'+result;
			}
		}
